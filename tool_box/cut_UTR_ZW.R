...
#script to remove UTRs of coding sequences in ZW species
...
# Import the gtf file 
fichier_gtf <- "/media/tallon@newcefe.newage.fr/DATAS/genome_reference/Artemia_sinica_annotation.gtf"
...
donnees_gtf <- read.table(fichier_gtf, sep="\t", header=FALSE, comment.char="#", stringsAsFactors=FALSE)

# Add colum names
colnames(donnees_gtf) <- c("Chromosome", "Source", "Type", "Start", "End", "Score", "Strand", "Phase", "Attributes")

# Split the Attributes column into separate columns
donnees_gtf <- donnees_gtf %>%
...
donnees_gtf$ID <- gsub("ID ", "", donnees_gtf$ID)
donnees_gtf$gene_ID <- gsub("gene_id ", "", donnees_gtf$gene_ID)
donnees_gtf$transcript_ID <- gsub("transcript_id ", "", donnees_gtf$transcript_ID)
donnees_gtf$Parent <- gsub("Parent ", "", donnees_gtf$Parent)
