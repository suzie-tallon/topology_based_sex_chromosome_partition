...
#script use to divide a fasta file with many genes into files with one gene per file with the X and Y sequence
...
mkdir fasta_to_cut
mv *_X.fasta fasta_to_cut
cd fasta_to_cut

# rename file
for file in *_X.fasta; do
    mv "$file" "${file/_X/}"
done
